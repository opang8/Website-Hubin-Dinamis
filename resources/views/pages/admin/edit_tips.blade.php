        ...
        <div class="col-lg-12 mb-4">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-success">Edit Tips </h6>
            </div>
            <div class="card-body">
             @foreach ($tips as $item)
                <form action="{{ route('action-table4.update', $item->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    
                    <div class="form-group mt-1">
                        <label for="judul" class="font-weight-bold">Judul</label>
                        <input type="text" class="form-control" name="judul" placeholder="" id="judul" required value="{{  $item->judul }}">
                    </div>

                    <div class="form-group">
                        <label for="gambar">Gambar</label>
                        <input type="file" class="form-control-file" id="gambar" name="gambar" required value="{{ $item->gambar }}">
                    </div>
            
                     
                     <div class="form-group">
                      <label for="exampleFormControlFile1" class="font-weight-bold">Text</label>
                       <textarea class="ckeditor" id="ckedtor" name="text" value="">{{  $item->text }}</textarea>
                    </div>
